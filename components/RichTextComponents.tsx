...
export const RichTextComponents = {
  ...
  block: {
    ...
    blockquote: ({ children }: any) => (
      <blockquote className="border-l-4 border-[#2cf608] pl-5 py-5 my-5">{children}</blockquote>
    )
  },

  marks: {
    link: ({ children, value }: any) => {
      const rel = !value.href.startWith("/") ? "noopener noreferrer" : undefined;

      return (
        <Link
          href={value.href}
          rel={rel}
          className="underline decoration-[#2cf608] hover:decoration-black"
        >
          {children}
        </Link>
      );
